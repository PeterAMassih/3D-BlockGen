...
class DiffusionInference3D:
    ...
    def sample(self, num_samples=8, image_size=(32, 32, 32), show_intermediate=False):
    ...
    def sample_ddim(self, num_samples=8, image_size=(32, 32, 32), num_inference_steps=50, show_intermediate=False):
    ...
    def visualize_samples(self, samples, threshold=0.5):
        ...
        for i, sample in enumerate(samples_):
            binary_sample = (sample > threshold).astype(bool)
            
            # Visualize center slice
            axs[0, i].imshow(binary_sample[0, :, :, sample.shape[3]//2], cmap="gray")
            ...
            ax.voxels(binary_sample[0], edgecolor='k') # Because binary channel only one channel here [0]

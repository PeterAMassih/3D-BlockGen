...
runai submit $arg_job_name \
  ...
  --command -- /bin/bash -c "\". /opt/lab/setup.sh && cd /scratch/students/2024-fall-sp-pabdel/3D-BlockGen && su $CLUSTER_USER -c 'pip install -e .' && su $CLUSTER_USER -c 'python scripts/process_dataset_clean.py' \""
